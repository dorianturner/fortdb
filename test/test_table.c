...
int main(void) {
    printf("About to try and make a table \n");
    fflush(stdout);
    ...
    /* ---------------- Basic set/get ---------------- */
    assert(table_set_field(t, "users/alice/profile/main/name", "Alice", 1) == 0);
    assert(table_set_field(t, "users/alice/profile/main/age", "30", 2) == 0);
    printf("Table values set successfully \n");
    fflush(stdout);

    // First writes are always local version 0
    char *name = table_get_field(t, "users/alice/profile/main/name", 0);
    char *age  = table_get_field(t, "users/alice/profile/main/age", 0);
    ...
    /* ---------------- Versioning ---------------- */
    assert(table_set_field(t, "users/alice/profile/main/name", "Alice V2", 3) == 0);

    // Local version 0 -> "Alice", local version 1 -> "Alice V2"
    char *name_v0 = table_get_field(t, "users/alice/profile/main/name", 0);
    char *name_v1 = table_get_field(t, "users/alice/profile/main/name", 1);
    ...
    /* ---------------- Deletion ---------------- */
    assert(table_delete_path(t, "users/alice/profile/main/age", 4) == 0);
    char *age_after_delete = table_get_field(t, "users/alice/profile/main/age", 1);
